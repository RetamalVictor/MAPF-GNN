...
class DotsEnv(gym.Env):
    ...
    def __init__(self, nb_agents, agent_random_start=False, speed=0.6):
        ...
        self.action_space = spaces.Box(low=-1, high=1, shape=(nb_agents,), dtype=np.float32)
        self.observation_space = spaces.Box(low=-15, high=15,
                                            shape=(nb_agents*2,), dtype=np.float32)
    ...
    def reset(self):
        ...
        if self.agent_random_start:
            self.posX = np.array([np.random.randint(3,6) * np.random.choice([-1,1]) for i in range(self.nb_agents)], dtype=np.float32) 
            self.posY = np.array([np.random.randint(3,6) * np.random.choice([-1,1]) for i in range(self.nb_agents)], dtype=np.float32) 
        else:
            if self.nb_agents == 2:
    ...
    def check_boundary(self):
        if np.any(self.posX > 12) or np.any(self.posX < -12) or np.any(self.posY > 12) or np.any(self.posY < -12):
            self.bound = True
            return True
        else: 
            return False
    ...
    def printNeighbours(self):
        for agent in self.agents:
            print(f"Agent {agent} has {self.agents[agent].getNbNeighbours()} neighbours")
...
if __name__ == '__main__':
    ...
    env.reset()
    for i in range(200):
        action = {str(i): [np.random.uniform(-1,1),np.random.uniform(-1,1)] for i in range(age)}
