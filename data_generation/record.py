...
if __name__ == "__main__":
    ...
    total=100
    pwd_path = fr"dataset\2_0_5"
    t = np.zeros(total)
    for i in range(total):
        trayectory = np.load(os.path.join(pwd_path,fr"case_{i}\trayectory.npy"), allow_pickle=True)
    ...
    for timestep in range(total):
        ...
        env = make_env(os.path.join(pwd_path, fr"case_{timestep}"))
        trayectory = np.load(os.path.join(pwd_path,fr"case_{timestep}\trayectory.npy"), allow_pickle=True)
        ...
        np.save(os.path.join(pwd_path,fr"case_{timestep}\states.npy"), recordings)
        np.save(os.path.join(pwd_path,fr"case_{timestep}\embeddings.npy"), emb_record)
        np.save(os.path.join(pwd_path,fr"case_{timestep}\gso.npy"), adj_record)
        np.save(os.path.join(pwd_path,fr"case_{timestep}\trajectory.npy"), trayectory)
        if timestep%25 == 0:
            print(f"[{timestep}/{total}] --- completed")
    print(f"[{timestep}/{total}] --- completed")
