...
def record_env(path, config):
    ...
    for timestep in range(len(cases)-1):
            ...
            env = make_env(os.path.join(path, fr"case_{timestep}"), config)
            # mx = env.min_time
            trayectory = np.load(os.path.join(path,fr"case_{timestep}\trajectory.npy"), allow_pickle=True)
            trayectory = trayectory[:,1:]
            recordings = np.zeros((trayectory.shape[1], agent_nb, 2, 5, 5)) # timestep, agents, channels of FOV, dimFOVx, dimFOVy
            ...
            np.save(os.path.join(path,fr"case_{timestep}\trajectory_record.npy"), trayectory)
            if timestep%25 == 0:
...
if __name__ == "__main__":
    ...
    # total=200
    pwd_path = fr"dataset\5_7_16\test"
    record_env(pwd_path)
